	...
	<view class="questions">
		<view class="test-header">
			<view @click="showModal">
				<view>
					<image src="../../../static/onlineTestImgs/jiaojuan.png"></image>
				</view>
				<text>交卷</text>
			</view>
			<view>倒计时 10:00</view>
			...
			<swiper-item v-for="(item,index) in lists" :key="index" class="swiper-item">
				<view class="test-main">
					...
					<view class="test-cont">
						<view v-for="(item2,index2) in item.testList" :key="index2" @click="testItem(index,index2,item.code)" :class="{active_true:(!item.code2 && item2.code) || (item.code == 2 && item2.click_index),active:(item.code == 1 && item2.click_index && !item2.code) || (item.code == 2 && item2.click_index && !item2.code && !item.code2)}">
							<view>{{item.option[index2]}}</view>
							<text v-if="item.isimg == 1">{{item2.name}}</text>
							<image v-else src="../../../static/onlineTestImgs/banner01.jpg" mode="aspectFit"></image>
						</view>
					</view>
				</view>
		...
		<view class="fixed-bottom" :class="{active:show_hide}" @click.stop="hideFixed">
			<view class="tibiao" @click.stop>
				...
				<view>
					<text v-for="(item,index) in lists.length" :key="index" :class="{active_o:lists[index].topic && !lists[index].code2,active_t:!lists[index].topic && !lists[index].code2}"
					 @click="topic(index)">{{index+1}}</text>
				</view>
			</view>
		</view>
	</view>
